...
export class MenuBar extends Disposable {
	...
	createOverflowMenu(): void {
		...
		const buttonElement = $('div.cet-menubar-menu-button.more', { 'role': 'menuitem', 'tabindex': this.isCompact ? 0 : -1, 'aria-label': label, 'title': title, 'aria-haspopup': true });
		const titleElement = $('div.cet-menubar-menu-title.toolbar-toggle-more', { 'role': 'none', 'aria-hidden': true });
	...
	private showCustomMenu(menuIndex: number, selectFirst = true): void {
		...
		console.log('Index', actualMenuIndex);

		if (!customMenu.actions || !customMenu.buttonElement) {
			return;
		}

		const menuHolder = $('ul.cet-menubar-menu-container', { 'title': '' });
